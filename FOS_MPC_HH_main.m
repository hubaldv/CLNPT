...
addpath('KovalskyModel')
addpath('KovalskyModel/gating')
...
load('KovalskyModel/HH_data.mat');
signal = HH_data(2,:)';
t_vec = HH_data(1,:)';
...
xlabel('Time [ms]');
ylabel('Potential [mV]');
leg = legend('Kovalsky model output','Interval used for FOS system ID');
...
% simulation horizon
N = 3500*freq; % Simulation horizon (sampling freq = 100000Hz)
...
uuu0Nminus1 = zeros(nu,N);
xxx1N_OBS = zeros(p,N);
xxx1N_NO_OBS = zeros(p,N);
xxx1N_OLD = zeros(p,N);
k = 0;
xxxkplusj_OBS = xxx0;
xxxkplusj_NO_OBS = xxx0;
xxxkplusj_OLD = xxx0;
xxx1N_OBS(:,1) = xxxkplusj_OBS;
xxx1N_NO_OBS(:,1) = xxxkplusj_NO_OBS;
xxx1N_OLD(:,1) = xxxkplusj_OLD;
...
HH = [V(1); mf(1); hf(1); ml1(1); hl1(1); ml2(1); hl2(1); n_K(1); nnorm(1)];
...
dt = 0.01;           % Sample time for the model
Ibase = 0.5;         % Input to the model, use this parameter to add/remove activity

controller_start = 1000;
controller_stop = 2500;
...
%     uuu0Pminus1 = fmincon(fun, zeros(P,1),[],[],[],[], umin*ones(P*nu,1), umax*ones(P*nu,1), [], options);
    end
    ...
    for j=0:(M-1)
        ...
        uuu0Nminus1(:,i) = uuukplusj;
        
        % Update real model output
%         w2 = rand(1)*20;  
        Istim = Ibase;
        xk_HH_OL = HH_step(xk_HH_OL, dt, Istim);
        xN_HH_OL(:,i) = xk_HH_OL;
        Istim = Ibase + uuukplusj;
        xk_HH_CL = HH_step(xk_HH_CL, dt, Istim);
        xN_HH_CL(:,i) = xk_HH_CL;
        ...
        xxx1N_OBS(:,i) = xxxkplusj_OBS;
        xxx1N_NO_OBS(:,i) = xxxkplusj_NO_OBS;
%         if ~controller_active
%             xxx1N_OBS(:,end) = NaN;
%         end
        xxx1N_OLD(:,i) = xxxkplusj_OLD;
        k = k + 1;          
    end
...
%         w2 = rand(1)*20;
        ...
        w1 = normrnd(0,0,[n,1]);
        xxxkplusj_OBS = AAA*xxxkplusj_OBS + L * ([1 0 0 0 0 0 0 0 0]*xk_HH_CL - ccc*xxxkplusj_OBS) + BBB*uuukplusj+BBBw*w1;
        xxxkplusj_NO_OBS = AAA*xxxkplusj_NO_OBS + BBB*uuukplusj + BBBw*w1;
        xxxkplusj_OLD = AAA*xxxkplusj_OLD + BBBw*w1;
...
x1N_HH_OL = xN_HH_OL(1:n,:);
x1N_HH_CL = xN_HH_CL(1:n,:);
...
h(1) = plot(0:(1/freq):(length(x1N_HH_OL(1,:))/freq),[HH_x0(1) x1N_HH_OL],'-','Color',[1 0 0.22],'LineWidth',1);
h(2) = plot(0:(1/freq):(length(x1N_HH_CL(1,:))/freq),[HH_x0(1) x1N_HH_CL],'-','Color',[0 0.28 0.67],'LineWidth',1);
xlim([0 (length(x1N_HH_OL(1,:))/freq)])
ylim([-80 40]);
