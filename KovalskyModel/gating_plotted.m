%% Plot also the gating variables
clear; clc; close;
...
figure('Renderer', 'painters', 'Position', [1000 1000 1200 400]);
...
% Fast
subplot(3,3,2);
plot(t,mf,'Color',[0.07 0.53 0.03]);
grid on;
axis([min(t) max(t) 0 1]);
ylabel('$m_{F}(t)$',format{:},'Color',[0.07 0.53 0.03]);

subplot(3,3,3);
plot(t,hf,'Color',[1 0 0.22]);
grid on;
axis([min(t) max(t) 0 1]);
ylabel('$h_{F}(t)$',format{:},'Color',[1 0 0.22]);

% Late 1
subplot(3,3,5);
plot(t,ml1,'Color',[0.07 0.53 0.03]);
grid on;
axis([min(t) max(t) 0 1]);
ylabel('$m_{M}(t)$',format{:},'Color',[0.07 0.53 0.03]);

subplot(3,3,6);
plot(t,hl1,'Color',[1 0 0.22]);
grid on;
axis([min(t) max(t) 0 1]);
ylabel('$h_{M}(t)$',format{:},'Color',[1 0 0.22]);

% Late 2
subplot(3,3,8);
plot(t,ml2,'Color',[0.07 0.53 0.03]);
grid on;
axis([min(t) max(t) 0 1]);
ylabel('$m_{S}(t)$',format{:},'Color',[0.07 0.53 0.03]);
xlabel('Time [ms]',format{:});
