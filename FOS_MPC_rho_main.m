...
for i = 0:lags
    cor = autocor(i+1);
    plot([i, i], [0, cor], '-','Color',[1 0 0.22],'LineWidth',0.25);
    plot([i, i], [cor, cor], 'o','Color',[1 0 0.22],'LineWidth',0.25);
end
h1 = plot([0 lags], [confidence_95 confidence_95],'-','Color',[0 0.18 0.97],'LineWidth',1);
plot([0 lags], -[confidence_95 confidence_95],'-','Color',[0 0.18 0.97],'LineWidth',1);
xlabel('Lag [samples]','Interpreter','latex','fontsize',18);
ylabel('Sample autocorrelation','Interpreter','latex','fontsize',18);
leg = legend(h1, '95\% confidence interval');
